...
export default {
    ...
    filterData() {
      if (this.searchText) {
        return this.products.filter((item) => {
          const data = item.title.toLowerCase().includes(this.searchText.toLowerCase());
          return data;
        });
      }
  ...
  methods: {
    // 有傳參數的不可使用 mapActions 展開方法
    addtoCart(id, qty = 1) {
      console.log(this.$store.state.products) // undefined 為 products.js 區域變數 這裡不認得
      console.log(this.$store.state.productsModule.products) // 帶上 products.js 模組名稱即可取得模組內資料
  ...
  created() {
    ...
    this.getProducts();// 引用模組之後仍可以透過 this + mapActions 呼叫模組內方法
    // console.log(this.$store.getters)// 可以透過此方式取得getters內容
  }
};
</script>
